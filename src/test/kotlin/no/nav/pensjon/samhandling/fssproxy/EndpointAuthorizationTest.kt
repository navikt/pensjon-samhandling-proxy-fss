...
@SpringBootTest(
    ...
    classes = arrayOf(PensjonPenProxyFssApplication::class))
@AutoConfigureMockMvc
@Import(RestTemplateTestConfig::class)
class EndpointAuthorizationTest(@Autowired private val mockMvc: MockMvc, @Autowired restTemplate: RestTemplate,
    ...
    companion object {
        ...
        private fun azureAdClientCredentials() = Stream.of(
            ...
            ClientCredentialsAuthorization(HttpMethod.GET, "folketrygdbeholdning/beregning", listOf("folketrygdbeholdning")),

        )

        @JvmStatic
        private fun azureAdOnBehalfOf() = Stream.of(
            OnBehalfOfAuthorization(HttpMethod.GET, "samordningsgrunnlag", listOf("krigspensjon"), listOf(krigspensjonGroupId))
        )

        @JvmStatic
        private fun maskinporten() = Stream.of(
            MaskinportenAuthorization(HttpMethod.GET, "vedtak/ytelse", listOf("nav:pensjon/v1/ytelse")),
            MaskinportenAuthorization(HttpMethod.GET, "vedtak/ytelsehistorikk", listOf("nav:pensjon/v1/ytelsehistorikk")),
    ...
    //Client credentials
    @ParameterizedTest
    @MethodSource("azureAdClientCredentials")
    fun `test azure endpoint authorization with correct role, is ok`(authorization: ClientCredentialsAuthorization) {
        mockPen.setupOkResponse(authorization.method, authorization.path)

        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createAzureAdAuthHeader(audience = "pensjon-pen-proxy-fss", roles = authorization.roles)))
            .andExpect(MockMvcResultMatchers.status().isOk)
    }

    @ParameterizedTest
    @MethodSource("azureAdClientCredentials")
    fun `test azure endpoint authorization with wrong role, is forbidden`(authorization: ClientCredentialsAuthorization) {
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createAzureAdAuthHeader(audience = "pensjon-pen-proxy-fss",  roles = listOf("very-wrong-role"))))
    ...
    fun `test azure endpoint authorization with correct scope and group, is ok`(authorization: OnBehalfOfAuthorization) {
        ...
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createAzureAdAuthHeader(audience = "pensjon-pen-proxy-fss",  scopes = authorization.scopes,
    ...
    fun `test azure endpoint authorization with wrong scope but correct group, is forbidden`(authorization: OnBehalfOfAuthorization) {
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createAzureAdAuthHeader(audience = "pensjon-pen-proxy-fss",  scopes = listOf("very-wrong-scope"), groups = authorization.groups)))
            .andExpect(MockMvcResultMatchers.status().isForbidden)
    }
    ...
    fun `test azure endpoint authorization with correct scope but wrong group, is forbidden`(authorization: OnBehalfOfAuthorization) {
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createAzureAdAuthHeader(audience = "pensjon-pen-proxy-fss",  scopes = authorization.scopes, groups = listOf("very-wrong-group"))))
            .andExpect(MockMvcResultMatchers.status().isForbidden)
    }
    ...
    fun `test maskinporten endpoint authorization with correct scope is ok`(authorization: MaskinportenAuthorization) {
        ...
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createMaskinportenAuthHeader(scopes = authorization.scopes)))
            .andExpect(MockMvcResultMatchers.status().isOk)
    }
    ...
    fun `test maskinporten endpoint authorization with wrong scope is forbidden`(authorization: MaskinportenAuthorization) {
        this.mockMvc
            .perform(
                requestBuilder(method = authorization.method, path = authorization.path, createMaskinportenAuthHeader(scopes = listOf("very-wrong-scope"))))
            .andExpect(MockMvcResultMatchers.status().isForbidden)
    }
    ...
    private fun createAzureAdAuthHeader(roles: List<String> = emptyList(), audience: String, scopes: List<String> = emptyList(), groups: List<String> = emptyList()) =
        "Bearer ${
            trustedAzureAdAuthenticationServer.generateClientJWT(
            audience = audience, roles = roles, scopes = scopes, groups = groups)}"

    private fun createMaskinportenAuthHeader(roles: List<String> = emptyList(), scopes: List<String> = emptyList(), groups: List<String> = emptyList()) =
        "Bearer ${
